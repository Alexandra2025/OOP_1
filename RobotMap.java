...
public class RobotMap {
    ...
    private int limitRobots = 5; // количество роботов не больше по умолчанию
    private final int n;
    private final int m;
    ...
    // Проверка количества создаваемых роботов
    private void checkLimitRobots() {
        if (robots.size() == limitRobots) {
            throw new IllegalStateException("Слишком много роботов!");
        }
    }

    // Проверка данных создаваемой карты
    private void validateSizeMap(int n, int m) {
        if (n <= 0 || m <= 0) {
            throw new IllegalStateException("Размер карты задан неверно!");
        }
    }
    
    // Проверка заданной точки
